...
while (<$INPUT>) {
	...
	system("/public2/home/zhaoch/local/samtools-1.3.1/samtools view $bamfile $bamposition > $TEMPNAME"); #get all reads overlapping position
	my $editnuc = $fields[4];
	...
	while (<$TMPFILE>) { #loop through the reads
		...
		if (($revstrand == 0 && $base_readpos > 10) || ($revstrand == 1 && $base_readpos < $readpos - 9)) { #ensure that this position is not in first 6 bp of read
