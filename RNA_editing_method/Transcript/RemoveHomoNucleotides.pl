...
my $tmpfile = join '', $inputfile, '.nuctmp';#生成变异位点前后4bp的bed文件
...
while(<SITES>) { #Read INPUTFILE
	...
	system("/public2/home/zhaoch/local/bedtools2/bin/fastaFromBed -fi $Reference -bed $tmpfile -fo $tmpfafile"); #get sequence surrounding candidate position
	my $seq = '';
	...
	print OUTPUT "$line\n" unless ($homopol); #check if homopolymer #若此变异位位于连续5bp的变异位点中，则去掉此位点
}
