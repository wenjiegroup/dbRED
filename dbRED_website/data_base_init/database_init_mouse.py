...
django.setup()
from RnaSearch.models import Mouse_Rnaedit
...
def main():
    ...
    for i in f.readlines():
        ...
        if lis[11] == 'synonymous SNV' or lis[11] == 'nonsynonymous SNV' or lis[11] == 'stoploss':
            ...
            if 'ENSMUST' in lis[12]:
                Genename = lis[12].split('(')[0]
                Genedetail = lis[12].split('(')[1].split(')')[0]
            else:
                Genename = lis[12]
                Genedetail = None
        Editlevel = float(lis[14])

        Mouse_Rnaedit.objects.create(name=name, Chr = lis[0], position = int(lis[1]),\
        ...
        circRNA = lis[18], Sequence = lis[19], Cellline = lis[20])
        n += 1
        print n
    f.close()
